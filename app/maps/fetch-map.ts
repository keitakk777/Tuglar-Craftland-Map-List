    ...
    for (let i = headerIdx + 1; i < rows.length; i++) {
      ...
      if (!mapCode && rawLink.includes("m=")) {
        ...
        if (match && match[1]) {
           ...
           mapCode = coreCode.length > 10 ? "#FREEFIRE" + coreCode : "#" + coreCode;
        }
      }
      ...
      if (mapCode && !mapCode.startsWith("#")) {
          mapCode = (mapCode.length > 10 && !mapCode.toUpperCase().startsWith("FREEFIRE")) ? "#FREEFIRE" + mapCode : "#" + mapCode;
      }
      ...
      maps.push({
        ...
        link: rawLink,      
        playLink: rawLink, 
        image: getDirectImageUrl(rawImage),
